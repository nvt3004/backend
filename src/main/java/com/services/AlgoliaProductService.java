...
public class AlgoliaProductService {
	...
	public List<ProductDTO> getTopProducts() {
		List<String> att = Arrays.asList("id", "name", "imgName", "rating", "minPrice", "maxPrice", "reviewCount");
		Query query = new Query().setQuery("") 
				.setAttributesToRetrieve(att).setHitsPerPage(12);

		try {
			SearchResult<ProductDTO> searchResult = productIndex.search(query);
			return searchResult.getHits(); 
		} catch (Exception e) {
			logger.severe("Lỗi khi lấy sản phẩm tốt nhất từ Algolia: " + e.getMessage());
			return List.of(); 
		}
	}
